...
func (p projectInfo) Create(obj interface{}) (interface{}, error) {
	...
	pagerRecord, _ := p.pagerClient.DevopsV1alpha1().Pagers(syncer.DevopsNamespace).Get(ctx, "workspace-"+workspaceName, v1.GetOptions{})
	...
	project, resp, err := p.gitlabClient.Projects.CreateProject(&git.CreateProjectOptions{
		Name:                                name,
		Path:                                name,
		NamespaceID:                         groupID,
		DefaultBranch:                       nil,
	...
	_, err = p.pagerClient.DevopsV1alpha1().Pagers(syncer.DevopsNamespace).Create(ctx, &v1alpha1.Pager{
